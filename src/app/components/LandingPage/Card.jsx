  ...
  const cardData = [
    {
      ...
      description:
        "To ensure accuracy, all genealogical data is manually verified, giving users confidence in the reliability of the information provided.",
    },
    {
      image: card2,
      title: "Expandable Family Trees",
      description:
        "This feature allows users to build and expand their family trees, adding new branches as they discover more ancestors and relatives.",
    },
    {
      image: card3,
      title: "Free Forever Access",
      description:
        "Users are granted lifetime access to the website's resources and features without any charges, ensuring that everyone can explore their ancestry at no cost.",
    },
    {
      image: card4,
  ...
  return (
    <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 px-4 py-12 bg-[#F4FFE8]">
      {cardData.map((card, index) => (
        <div
          key={index}
          className="shadow-lg p-4 rounded-lg bg-white my-16 mx-3 "
        >
          <Image
            src={card.image}
          ...
          <p className="text-sm text-left text-[#888888]">{card.description}</p>
        </div>
      ))}
    </section>
  );
