...
import { windsorData } from "../../../../utils/const/index";

export const ForceLayout = (props) => {
  const { width } = useWindowDimensions();
  const famTreeRef = useRef(null);

  const { width: famTreeWidth, height: famTreeHeight } =
    useContainerDimensions(famTreeRef);
  ...
  const names = ["------"].concat(
    nodesData
      .filter(function (d) {
        return d.name && d.type != "marriage";
      })
      .map(function (d) {
        if (d.type === "person") {
          var liveString;
          if (d.died && d.died != "alive")
            liveString = "(" + d.birth + "-" + d.died + ")";
          else if (!d.died || d.died === "alive")
            liveString = "(Born : " + d.birth + ")";
    ...
    (svg) => {
      ...
      nodesData.forEach((dNode) => {
        ...
        if (dNode.type === "person") {
          if (
            dNode.name === "Queen Victoria" ||
            dNode.name === "Prince Albert"
          ) {
      ...
      const simulation = d3
        ...
        .force(
          "link",
          d3.forceLink(linksData).id((d) => {
            /*console.log(d);*/ return d.id;
          })
        )
        .force("charge", d3.forceManyBody().strength(-150))
        .force("x", (d) => {
          /*console.log(d3.forceX());*/ return d3.forceX();
        })
        .force("y", d3.forceY());
        ...
        .join(
          ...
          (update) =>
            update
              ...
              .style("fill", "none"),
          (exit) =>
            exit.transition().duration(250).style("stroke-width", 0).remove()
        );
      ...
      const masks = focus
        ...
        .join(
          (enter) =>
            enter
              ...
              .attr("width", function (d) {
                if (
                  d.name === "Charles Carnegie" ||
                  d.name === "Friedrich Josias"
                )
                  return 100;
                return 200;
              })
          ...
          (exit) => exit.transition().duration(250).remove()
        );

      const node = focus
        .selectAll(".nodes")
        .data(
          nodesData.filter(function (d) {
            return d.name;
          }),
          function (d) {
            return d.id;
        ...
        .join(
          (enter) => {
            ...
            g.append("rect")
              ...
              .duration(250)
              .attr("width", function (d) {
                return d.type === "person" ? 10 : d.type === "house" ? 0 : 1.75;
              })
              .attr("height", function (d) {
                return d.type === "person"
                  ? 12.5
            ...
            g.append("image")
              ...
              .attr("transform", function (d) {
                if (d.type === "person") {
                  ...
                  else return "translate(-3.75, -5) scale(0.0375)";
            ...
            g.append("text")
              ...
              .attr("font-size", function (d) {
                return d.type === "person" || d.type === "house"
                  ? "3px"
                  : "0px";
              })
              .attr("transform", function (d) {
                ...
                if (d.type === "person") {
                  ...
                  if (
                    d.name === "Queen Victoria" ||
                    d.name === "Prince Albert"
                  ) {
                    //if(d.name === "Queen Victoria")xPos = -60
                    //if(d.name === "Albert")xPos = -20;
                    yPos = 10;
              ...
              .style(
                "text-shadow",
                "0 1px 0 #f8fafc, 1px 0 0 #f8fafc, -1px 0 0 #f8fafc, 0 -1px 0 #f8fafc"
              );
            ...
            g.append("text")
              ...
              .text(function (d) {
                ...
                if (d.type === "person") {
                  if (d.name.indexOf(" ") > 1)
                    titleToDisplay = d.name.substring(
                      d.name.indexOf(" "),
                      d.name.length
                    );
                  else if (
      ...
      node
        ...
        .style("pointer-events", "none")
        .filter(function (d) {
          return d.divorce === true;
        })
        .style("stroke-dasharray", "1 1");

      nodes
        .filter(function (d) {
          return d.type === "marriage" && d.divorce;
        })
        .forEach(function (dNode) {
          link
            .filter(function (dLink) {
              return dLink.target.name === dNode.name;
            })
      ...
      function positionLinks() {
        link
          .filter(function (d) {
            return d.id === "bride-left";
          })
          .attr("d", function (d) {
            return (
              "M " +
              d.source.x_pos * responsiveXPos +
              " " +
              calculateYPos(d.source) +
              " L " +
              d.source.x_pos * responsiveXPos +
              " " +
              calculateYPos(d.target) +
              " " +
              d.target.x_pos * responsiveXPos +
              " " +
              calculateYPos(d.target)
            );
          });
        ...
        link
          ...
          .attr("d", function (d) {
            return (
              ...
              d.source.x_pos * responsiveXPos +
              " " +
              (calculateYPos(d.source) + 16.25 * responsiveYPos) +
              " " +
              (d.source.x_pos - 80) * responsiveXPos +
              " " +
              (calculateYPos(d.source) + 16.25 * responsiveYPos) +
              " " +
              (d.source.x_pos - 80) * responsiveXPos +
              " " +
              (calculateYPos(d.source) + 67.5 * responsiveYPos) +
              " " +
              (d.target.x_pos - 20) * responsiveXPos +
              " " +
              (calculateYPos(d.source) + 67.5 * responsiveYPos) +
              " " +
              (d.target.x_pos - 20) * responsiveXPos +
              " " +
              (calculateYPos(d.source) + 21.75 * responsiveYPos) +
              " " +
              d.target.x_pos * responsiveXPos +
              " " +
              (calculateYPos(d.source) + 21.75 * responsiveYPos) +
        ...
        link
          .filter(function (d) {
            return d.source.name === "Margaret and Gustaf";
          })
          .attr("d", function (d) {
            return (
              "M " +
              ...
              d.source.x_pos * responsiveXPos +
              " " +
              (calculateYPos(d.source) + 77.5 * responsiveYPos) +
              " " +
              (d.target.x_pos - 10) * responsiveXPos +
              " " +
              (calculateYPos(d.source) + 77.5 * responsiveYPos) +
              " " +
              (d.target.x_pos - 10) * responsiveXPos +
              " " +
              (calculateYPos(d.target) - 10 * responsiveYPos) +
              " " +
              d.target.x_pos * responsiveXPos +
              " " +
              (calculateYPos(d.target) - 10 * responsiveYPos) +
        ...
        link
          .filter(function (d) {
            return (
              d.source.id === "Arthur1883" && d.target.id === "Alexandra_Arthur"
            );
          })
          .attr("d", function (d) {
            return (
              ...
              d.source.x_pos * responsiveXPos +
              " " +
              (calculateYPos(d.source) + 30 * responsiveYPos) +
              " " +
              (d.target.x_pos + 170) * responsiveXPos +
              " " +
              (calculateYPos(d.source) + 30 * responsiveYPos) +
              " " +
              (d.target.x_pos + 170) * responsiveXPos +
              " " +
              (calculateYPos(d.target) + 8 * responsiveYPos) +
              " " +
              d.target.x_pos * responsiveXPos +
              " " +
              (calculateYPos(d.target) + 8 * responsiveYPos) +
      ...
      function position_EdwardVII_children() {
        ...
        link
          .filter(function (d) {
            return d.source.name === "Edward VII and Alexandra";
          })
          .attr("d", function (d) {
            var yDistance = calculateYPos(d.target) - calculateYPos(d.source);
      ...
      const yPadding =
        famTreeHeight > lowestLayerY ? (famTreeHeight - lowestLayerY) / 2 : 0;
      ...
      const calculateYPos = (d) => {
        ...
        if (d.layer) {
          ...
          if (d.layer === 3) {
            if (d["sub-layer"] === "a") {
              newY -= 35 * responsiveYPos;
            }
            if (d["sub-layer"] === "b") {
              newY += 5 * responsiveYPos;
            }
            if (d["sub-layer"] === "c") {
              newY -= 21.25 * responsiveYPos;
            }
            if (d["sub-layer"] === "d") {
              newY += 32.5 * responsiveYPos;
            }
          }

          if (d.layer === 4) {
            if (d["sub-layer"] === "c") {
              newY += 37.5 * responsiveYPos;
            }
          }
          ...
          if (d.layer === 7) {
            if (d["sub-layer"] === "a") {
              newY -= 25 * responsiveYPos;
            }
          }

          if (d.type === "marriage") newY += 10 * responsiveYPos;
          if (d.type === "house") newY += 20 * responsiveYPos;
          if (d.type === "house" && d.name === "House of Bourbon")
            newY -= 1.25 * responsiveYPos;
          if (d.type === "house" && d.id === "Bernadotte")
            newY -= 6 * responsiveYPos;
        }
      ...
      function nodeToHilit(theNode, idx) {
        var transformString1 = theNode
          .select("text.firstLabel")
          .attr("transform");
        var transformString2 = theNode
          .select("text.secondLabel")
          .attr("transform");
        //console.log(transformString.substring(transformString.indexOf("(")+1, transformString.indexOf(")")).split(","));
        var transformMatrix1 = transformString1
          .substring(
            transformString1.indexOf("(") + 1,
            transformString1.indexOf(")")
          )
          .split(",");
        transformMatrix1[0] = +transformMatrix1[0];
        transformMatrix1[1] = +transformMatrix1[1];
        var transformMatrix2 = transformString2
          .substring(
            transformString2.indexOf("(") + 1,
            transformString2.indexOf(")")
          )
          .split(",");
      ...
      function nodeMouseover(e, d, i) {
        ...
        const theNodeIdx = nodeSelection.data().findIndex(function (dNode) {
          return dNode.id === d.id;
        });

        if (nodeHovered === null) {
          nodeHovered = new nodeToHilit(theNode, theNodeIdx);
          if (nodeToSearch != null && nodeToSearch.id === nodeHovered.id)
            return;
      ...
      function nodeMousemove(e) {
        ...
        if (baseXPos < 0) {
          ...
          xPos = 0;
        } else if (baseXPos + offsetWidth > clientWidth) {
      ...
      function centerNZoom() {
        ...
        if (screenMode === "midLowScreen" && name.length > 3) {
          if (myName != name) {
            ...
            if (currentSearchIndex > namesToSearch.length - 1)
              currentSearchIndex = 0;
          }
          name = namesToSearch[currentSearchIndex];
        }
        if (name.length > 0 && names.indexOf(name) > -1) {
          //if name is listed in the tree...
          //first, hilit the node..
          let birthDate = "",
            nodeName = name;
          if (name.indexOf("(") > -1) {
            //persons listed with their living periods inside brackets...
            nodeName = name.substring(0, name.indexOf("(") - 1);
            if (name.indexOf("Born") > -1)
              //living person..
              birthDate = name.substring(
                name.indexOf(":") + 2,
                name.indexOf(")")
              );
            else {
          ...
          if (nodeToSearch === null) {
            ...
            if (birthDate === "") {
              //if searched node is a house...
              const theNode = nodeSelection.filter(function (d) {
                return d.name === nodeName;
              });

              nodeToSearch = new nodeToHilit(theNode, idxToSearch);
            } else {
              //if searched node is a person, it will have a birth date...
              const theNode = nodeSelection.filter(function (d) {
                return d.name === nodeName && d.birth === birthDate;
              });
          ...
          if (nodeHovered != null && nodeHovered.id === nodeToSearch.id)
            alreadyHilit = true;

          var theNode = nodeSelection.filter(function (d) {
            return d.id === nodeToSearch.id;
          });
          const theNodeIdx = nodeSelection.data().findIndex(function (d) {
            return d.id === nodeToSearch.id;
          });
          ...
          var transformMatrix = transformString
            .substring(
      ...
      function hilit(theNode, idx) {
        ...
        if (nodeToSearch != null && id === nodeToSearch.id) {
          if (name === "Queen Victoria" || name === "Prince Albert") {
            ...
            if (labelPos === "top") {
              //label positioned above picture
              theNode
                .select("text.firstLabel")
                .transition()
                .duration(100)
              ...
              //label positioned below picture
              theNode
                .select("text.firstLabel")
                .transition()
                .duration(100)
        ...
        if (nodeHovered != null && id === nodeHovered.id) {
          if (name === "Queen Victoria" || name === "Prince Albert") {
            ...
            if (labelPos === "top") {
              ...
              //name label positioned below picture
              theNode
                .select("text.firstLabel")
                .transition()
                .duration(100)
                ...
                .attr("transform", function (e, d) {
                  ...
                  if (id === "Hesse-DarmstadtA") {
                    transY -= 7;
                  }
                  if (id === "Hohenlohe-Langenburg") {
                    transX -= 15;
                    transY -= 10;
                  }
                  if (id === "Oldenburg") {
        ...
        theNode
          ...
          .duration(100)
          .attr("width", function (d) {
            return d.type === "person" ? 20 : d.type === "house" ? 0 : 1.75;
          })
          .attr("height", function (d) {
            return d.type === "person" ? 25 : d.type === "house" ? 0 : 1.75;
          })
          .attr("x", function (d) {
            return d.type === "person"
              ? -10
        ...
        theNode
          ...
          .attr("transform", function (d) {
            if (d.type === "person") {
              ...
              else return "translate(-7.5, -10) scale(0.0375)";
      ...
      function handleTooltip(d) {
        ...
        tooltip
          .select("#live")
          .style("display", d.type === "person" ? "block" : "none")
          .text("(" + liveString + ")");
        ...
        if (d.status === "monarch") {
          ...
          if (d["rule end"]) {
            rulingString =
              "Rule from " + d.crowned + " to " + d["rule end"] + ".";
          } else {
      ...
      function unhilit(theNode, idx) {
        ...
        theNode
          ...
          .attr("transform", function (d) {
            return (
              "translate(" + d.labelX.bottomLabelX + "," + bottomLabelY + ")"
            );
          });
        ...
        theNode
          ...
          .attr("height", function (d) {
            return d.type === "person" ? 12.5 : d.type === "house" ? 0 : 1.75;
          })
          .attr("x", function (d) {
            return d.type === "person"
              ? -5
  ...
  return (
    <div className="w-full h-full bg-green-400 flex-grow flex flex-col">
      <div className="h-[50px] grow-0 bg-slate-50 flex justify-start items-center">
        <form
          id="searchContainer"
          className="basis-full md:basis-1/2 flex justify-start items-center"
        >
      ...
      <div
        ref={famTreeRef}
        className="h-[calc(100%-50px)] bg-amber-400 flex overflow-hidden"
      >
      ...
      <div
        id="tooltip"
        className="fixed max-w-xs bg-yellow-300 bg-opacity-90 flex flex-col items-center"
      >
